...
def preprocess_image(image_path):
    """
    이미지 경로를 입력받아 모델 입력 형식에 맞게 전처리
    - 이미지 크기를 (224, 224)로 리사이즈
    - [0, 255] 범위를 [0, 1]로 정규화
    - 배치 차원 추가
    """
    img = tf.keras.utils.load_img(image_path, target_size=(224, 224))  # 이미지 로드 및 리사이즈
...
# 4. 이미지 경로 및 전처리
img_path = "./images/h_1.jpg"  # flower1.jpg의 경로 (사용자가 업로드한 이미지 경로)
img_array = preprocess_image(img_path)  # 전처리된 이미지 배열

# 5. 예측 수행
predictions = model.predict(img_array)  # 모델 예측
predicted_class_idx = tf.argmax(predictions[0]).numpy()  # 가장 높은 확률의 클래스 인덱스
